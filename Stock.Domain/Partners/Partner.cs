...
public class Partner : BaseEntity
{
    [Required]
        ...
        private Transaction CreateTransaction(IEnumerable<(Product product, int quantity)> items,
            TransactionType transactionType)
        {
            ...
            if (!items.Any() || items.Any(x => x.product == null || x.quantity < 1))
                throw new ArgumentException(
                    "List of items must be a non-empty list of non-null products and quantities of at least 1.",
                    nameof(items));
